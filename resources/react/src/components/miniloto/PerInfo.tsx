...
const PerInfo = ({ minilotoList, maxNum = 31 }: { minilotoList: MiniLoto[]; maxNum: number }) => {
  ...
  return (
    <>
      <div>
        過去10回でていない数字(ボーナス数字含む)：
        {prev10NotHitInBonusMap}
      </div>
      <div>
        過去5回でていない数字(ボーナス数字含む)：
        {prev5NotHitInBonusMap}
      </div>
      <div>
        過去10回でていない数字(ボーナス数字含まない)：
        {prev10NotHitMap}
      </div>
      <div>
        過去5回でていない数字(ボーナス数字含まない)：
        {prev5NotHitMap}
      </div>
    </>
  );
